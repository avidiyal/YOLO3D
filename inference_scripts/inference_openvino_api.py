...
#     # load weight
#     checkpoint = torch.load(reg_weights, map_location='cpu', weights_only=False)
...
#         # Run detection 2d
#         dets = detect2d(
#             weights='yolov5s_inference.onnx',
#             source=img_path,
...
# # Dataloader
#     dataset = LoadImages(source, img_size=imgsz, stride=32, auto=True)
#     # Run inference
...
#         # input_tensor = ov.Tensor(im.numpy())  # Convert to OpenVINO tensor
...
#     parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5s_inference.onnx', help='model path(s)')
...
#     parser.add_argument('--show_result', action='store_false', help='Show Results with imshow') # Changed store_true to store_false
...
def detect3d(
    ...
    for i, img_path in enumerate(imgs_path):
        ...
        # Run detection 2d
        dets = detect2d(
            weights='../weights/model_conversion/yolov5s.onnx',
            source=img_path,
            data='../data/coco128.yaml',
            imgsz=[640, 640],
            device='cpu',
...
def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5s.onnx', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'eval/image_2', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--data', type=str, default=ROOT / '../data/coco128.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640], help='inference size h,w')
    parser.add_argument('--device', default='cpu', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--classes', default=[0, 2, 3, 5], nargs='+', type=int, help='filter by class: --classes 0, or --classes 0 2 3')
    parser.add_argument('--reg_weights', type=str, default='../weights/model_conversion/resnet18.onnx', help='Regressor model weights')
